...
def main():
    ...
    while True:
        try:
            ...
            thread = threading.Thread(target=handle_client, args=(server_socket, data, client_addr), daemon=True)
            thread.start()
        except:
            time.sleep(1)
        ...
        case $ch in
            8)
                ...
                if [ -f /etc/dnstt/server.pub ]; then
                    ...
                    echo -e "${NEON_CYAN}║${NEON_GREEN}  $PUBKEY${NC}"
                    echo -e "${NEON_CYAN}╠═══════════════════════════════════════════════════════════════╣${NC}"
                    echo -e "${NEON_CYAN}║${NEON_WHITE}  Copy this key for your DNS client configuration${NC}"
                else
                    echo -e "${NEON_CYAN}║${NEON_RED}  No public key found! Please generate keys first.${NC}"
                fi
                echo -e "${NEON_CYAN}╚═══════════════════════════════════════════════════════════════╝${NC}"
...
if [ -f /etc/dnstt/server.key ]; then
    echo -e "${NEON_YELLOW}⚠️ Existing keys found, generating new ones...${NC}"
...
cd /etc/dnstt
echo -e "${NEON_CYAN}Generating new key pair...${NC}"

if /usr/local/bin/dnstt-server -gen-key -privkey-file server.key -pubkey-file server.pub 2>/dev/null; then
    echo -e "${NEON_GREEN}✅ Keys generated successfully with dnstt-server${NC}"
else
    echo -e "${NEON_YELLOW}⚠️ Using fallback key generation...${NC}"
    openssl genrsa -out server.key 2048 2>/dev/null
    openssl rsa -in server.key -pubout -out server.pub 2>/dev/null
...
echo -e "${NEON_CYAN}╔═══════════════════════════════════════════════════════════════╗${NC}"
echo -e "${NEON_CYAN}║${NEON_YELLOW}${BOLD}                    YOUR PUBLIC KEY                              ${NEON_CYAN}║${NC}"
echo -e "${NEON_CYAN}╠═══════════════════════════════════════════════════════════════╣${NC}"
...
echo -e "${NEON_GREEN}╠═══════════════════════════════════════════════════════════════╣${NC}"
echo -e "${NEON_GREEN}║${NEON_WHITE}  🔑 PUBLIC KEY:${NC}"
echo -e "${NEON_GREEN}║${NEON_CYAN}  $(cat /etc/dnstt/server.pub)${NC}"
if [ $OVERCLOCK_MODE -eq 1 ]; then
